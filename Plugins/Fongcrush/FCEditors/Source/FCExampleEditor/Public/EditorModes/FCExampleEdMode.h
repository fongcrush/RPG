 ...
 */
struct HFCPointProxy : public HHitProxy
{
	DECLARE_HIT_PROXY();

	HFCPointProxy(UObject* const& InRefObject, const int32& InIndex)
 ...
 * 
 */
class FCEXAMPLEEDITOR_API FFCExampleEdMode : public FEdMode
{
public:
	...
	//〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓〓
	FFCExampleEdMode();
	virtual ~FFCExampleEdMode() override;
	...
	/** 포인트 선택 */
	void SelectPoint(AFCExampleTargetPoint* const& TargetPoint, const int32& InIndex);
...
protected:
	...
	/** 현재 선택된 포인트 엑터 */
	TWeakObjectPtr<AFCExampleTargetPoint> CurrentSelectedPointActor;
